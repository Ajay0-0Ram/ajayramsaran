...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // spacing is responsive, bounded so it never gets silly
   const spacing = Math.max(100, Math.min(containerWidth / 5, 180));
    ...
    items.forEach((item, i) => {
        ...
        const visibleRange = window.innerWidth < 640 ? 1 : 3; // mobile vs desktop
        item.style.opacity = Math.abs(offset) > visibleRange ? 0 : 1;


        const caption = item.querySelector("p");
        if (caption) caption.style.display = offset === 0 ? "block" : "none";
    });
    }
